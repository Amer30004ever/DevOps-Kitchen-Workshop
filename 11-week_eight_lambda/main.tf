...
variable "owner" {
  default = "Your_First_Name" # Replace with your first name
}
...
resource "aws_lambda_function" "s3_copy_function" {
  filename         = "function.zip" # The deployment package (ZIP file) containing the Lambda code
  function_name    = "s3_copy_lambda" # The name of the Lambda function
